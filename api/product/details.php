...
if (isset($_GET["product_id"])) {
    ...
    if ($product) {
        ...
        $photosQuery = mysqli_query($connection, "SELECT * FROM product_images WHERE product_id ='$productId'");
        while ($photosData = mysqli_fetch_assoc($photosQuery)) {
            $photosData["image_url"] = "http://disscrounts.rw/" . $photosData["image_url"];
            $photos[] = $photosData;
        }
